...
const solutionsButton = await page.evaluateHandle(() => {
  return Array.from(document.querySelectorAll('[role="dialog"] button')).find(
    (b) => b.textContent?.includes("Solutions"),
  );
});
await solutionsButton.asElement()?.click();
await new Promise((r) => setTimeout(r, 100));

const companyButton = await page.evaluateHandle(() => {
  return Array.from(document.querySelectorAll('[role="dialog"] button')).find(
    (b) => b.textContent?.includes("Company"),
  );
});
await companyButton.asElement()?.click();
await new Promise((r) => setTimeout(r, 100));
